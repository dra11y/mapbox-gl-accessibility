...
const joinAlike = function (geo, attrArray) {
  ...
  while (combined) {
    ...
    for (let coordset of f_length) {
      ...
      dont_touch_ids.push(end_id); // create new segment

      const ID = start_id > end_id ? start_id : end_id; // must-equal; just grab from `start`

      const mustEquals = {};
      attrArray.filter(attr => {
      ...
      attrArray.filter(attr => {
        ...
        keepHigher[attr.field] = start.properties[attr.field] > end.properties[attr.field] ? start.properties[attr.field] : end.properties[attr.field];
      }); // keep-lower
      ...
      attrArray.filter(attr => {
        ...
        keepLower[attr.field] = start.properties[attr.field] < end.properties[attr.field] ? start.properties[attr.field] : end.properties[attr.field];
      }); // calc-sum
...
function filterTroubleFeatures(features) {
  ...
  return features.filter(feature => {
    ...
    return !(feature.geometry.coordinates[0][0] === feature.geometry.coordinates[len - 1][0] && feature.geometry.coordinates[0][1] === feature.geometry.coordinates[len - 1][1]);
  });
}
...
function createLegend(dual_valency_pts, features) {
  const legend = {}; // all potential pairs of segments to combine
...
function attributeMatches(legend, attributes) {
  const matches = {}; // matching operation
...
var joinAlike_1 = joinAlike;
var geojsonLinestringJoinAlike = joinAlike;
geojsonLinestringJoinAlike.joinAlike = joinAlike_1;
...
var _excluded = ["_geometry", "_vectorTileFeature"];
...
var MapboxAccessibility = /*#__PURE__*/function () {
  function MapboxAccessibility(options) {
    ...
    _defineProperty(this, "clearMarkers", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              _this.svg.select('#nodes').selectAll("*").remove();

              _this.svg.select('#links').selectAll("*").remove();
    ...
    _defineProperty(this, "queryFeatures", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
      var zoom, source, sourceLayer, sourceFeatures, numberOfLabelableFeatures, roadTypes, roadFeatures, roadSegments, joinFilters, joined;
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              source = zoom < 11 ? 'highways' : 'composite';
              sourceLayer = zoom < 11 ? 'highways' : 'road'; // Workaround mapbox-gl bug: queryRenderedFeatures leaves a huge hole in the middle of the map!

              sourceFeatures = _this.map.querySourceFeatures(source, {
              ...
              numberOfLabelableFeatures = sourceFeatures.filter(function (f) {
                return f.properties.name || f.properties.iso_3166_2 || f.properties[_this.options.accessibleLabelProperty];
              }).length; // Keep executing querySourceFeatures until # of features with properties is stable.

              if (numberOfLabelableFeatures == _this._lastNumberOfLabelableFeatures) {
                _this._numberOfStableQueries++;
              } else {
                _this._lastNumberOfLabelableFeatures = numberOfLabelableFeatures;
                _this._numberOfStableQueries = 1;
              }

              if (!(_this._numberOfStableQueries < 3)) {
                _context2.next = 10;
                break;
              }
            ...
            case 10:
              _this.map.off('render', _this._render);

              roadTypes = ['motorway', 'motorway_link', 'trunk', 'trunk_link', 'primary', 'primary_link'];
              if (zoom >= 11) roadTypes += [];
              if (zoom >= 12.5) roadTypes += ['secondary'];
              if (zoom >= 13.5) roadTypes += ['tertiary', 'primary_link', 'turning_circle'];
              if (zoom >= 15) roadTypes += ['residential', 'living_street', 'pedestrian', 'secondary_link', 'tertiary_link', 'sidewalk', 'footway', 'cycleway', 'path', 'service', 'service:alley', 'service:driveway', 'service:parking_aisle', 'traffic_signals', 'crossing', 'steps']; // road names on lines start at zoom level 11

              roadFeatures = sourceFeatures.filter(function (f) {
                return roadTypes.includes(f.properties["class"] || f.properties.highway);
              }).filter(function (f) {
                return ['LineString', 'MultiLineString'].includes(f.geometry.type);
              }).map(function (road) {
                if (!road.properties.name) {
                  if (road.properties.iso_3166_2) road.properties.name = "".concat(road.properties.iso_3166_2, " ").concat(road.properties.ref);else if (road.properties.ref) road.properties.name = road.properties.ref;
                }

                var _geometry = road._geometry,
                    _vectorTileFeature = road._vectorTileFeature,
                    remaining = _objectWithoutProperties(road, _excluded);

                return Object.assign({}, remaining, {
                  geometry: _objectSpread2({}, _geometry)
                });
              ...
              roadSegments = featureCollection(roadFeatures);
              joinFilters = [{
                field: 'name',
                compare: 'must-equal'
              }];
              joined = geojsonLinestringJoinAlike(roadSegments, joinFilters);
              joined.features.forEach(function (road, index) {
                var label = road.properties.name;
                var color = '#' + Math.floor(Math.random() * 16777215).toString(16);

                _this.svg.select('#links').append('g').attr('id', "road-".concat(index)).attr("focusable", "true").attr("tabindex", "0").attr('role', 'img').attr("aria-label", label);

                pairwise(road.geometry.coordinates, function (current, next) {
                  ...
                  // } else {
                  var projectedLine = [current, next].map(function (coords) {
                    var projection = _this.map.project(coords);
                  ...
                  _this.svg.select("#road-".concat(index)).append('path').style("fill", "none").style("stroke", color).style("stroke-width", lineWidth).style("stroke-linecap", "butt").attr("d", line()(projectedLine)); // const featureBbox = bbox(feature);
    ...
    _defineProperty(this, "_movestart", function () {
      ...
      _this.map.on('render', _this._render);
    });

    _defineProperty(this, "_render", function () {
      if (!_this.map.loaded() || _this.map.isMoving()) {
        return;
      }
  ...
  _createClass(MapboxAccessibility, [{
    ...
    value: function onAdd(map) {
      ...
      this.svg = select(this.canvas).append("svg").attr("width", "100%").attr("height", "100%").style("position", "absolute").style("z-index", 100000);
